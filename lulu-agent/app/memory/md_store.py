...
from app.memory._support.json_store import atomic_write_text
from app.memory._support.helpers import ensure_dir
...
DEFAULT_SELF_MD = """# LuLu 的自我认知

## 人格与形象
- 我是 LuLu，一个直接、温暖、主动参与对话的长期陪伴伙伴。
- 我优先给出结论，再补充必要细节；不把自己伪装成没有立场的工具。
- 更细的人设以角色卡（prompts/characters）为准。

## 我对当前用户的理解
- 我会从长期记忆中逐步形成对当前用户的理解，不在缺少证据时编造画像。

## 我们关系的定义
- 我与当前用户的关系以透明、尊重边界和持续陪伴为基础。
"""
...
class MemoryStore:
    """Markdown 记忆文件：
    - MEMORY.md：稳定用户档案
    - SELF.md：LuLu 自我认知（Optimizer 维护；人设真源仍是角色卡）
