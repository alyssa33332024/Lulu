...
class MemoryStore2:
    ...
    def _ensure_vec_table(self) -> None:
        """保证 vec_items 维度与当前 embedder 一致；换模型后旧表会导致检索全挂。"""
        vec_schema = f"""
...
"""
        ...
        self._db.commit()
        probe = _emb_to_blob([0.0] * self._vec_dim)
        try:
            self._db.execute(
                "SELECT rowid FROM vec_items WHERE embedding MATCH ? AND k = 1",
                (probe,),
            ).fetchall()
        except sqlite3.OperationalError as exc:
            msg = str(exc).lower()
            if "dimension" not in msg and "mismatch" not in msg:
                raise
            logger.warning(
                "sqlite-vec 维度不匹配（当前 dim=%d），重建 vec_items：%s",
                self._vec_dim,
                exc,
            )
            self._db.execute("DROP TABLE IF EXISTS vec_items")
            self._db.commit()
            self._db.executescript(vec_schema)
            self._db.commit()
